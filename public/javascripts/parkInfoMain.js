...
async function fetchParkDetails(parkId) {
  ...
  const todaysOperatingHoursText = document.getElementById(
  ...
  if (conditionCode) {
    ...
    let weatherImgOneSrcImg = await findCorrectIcon(
      weatherData.forecast.forecastday[1].day.condition.code
    );
    weatherImgOneSrc.src = weatherImgOneSrcImg;
    let weatherImgTwoSrcImg = await findCorrectIcon(
      weatherData.forecast.forecastday[2].day.condition.code
    );
    weatherImgTwoSrc.src = weatherImgTwoSrcImg;
    let weatherImgThreeSrcImg = await findCorrectIcon(
      weatherData.forecast.forecastday[3].day.condition.code
    );
  ...
  contactInformation.innerHTML =
    addressHeader + parkCodeHTML + addressHTML + phoneHTML + emailHTML;
