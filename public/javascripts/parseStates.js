...
export async function validateSearchInput(query) {
  ...
  const exactMatch = usStates.find(
    (state) => state.name.toUpperCase() === queryInput
  );
  ...
  const matchingStates = usStates.filter(
    (state) =>
      state.name.toUpperCase().includes(queryInput) || state.code === queryInput
  );

  if (matchingStates.length > 0) {
    const matchingStateNames = matchingStates
      .map((state) => state.name)
      .join(", ");
    const matchingStateCodes = matchingStates
